...
function removeWithId() {
    ...
    allRemoveButton.forEach((element) => {
        element.addEventListener("click", function (event) {
            ...
            let deletedElementId = event.currentTarget.parentNode.parentNode.getAttribute("data-id");
            // filter methodu serti odemeyen elementi cixar arrayden
            incomeArr = incomeArr.filter((x) =>
                x._id != deletedElementId
            );
            setLocalIncomeArr(JSON.stringify(incomeArr));
...
expenseForm.addEventListener("submit", function (event) {
    ...
    let localExpenseA = {
        _id: Math.random(),
        expenseName: incomeName.value.trim(),
        amount: incomeAmount.value.trim(),
        _userId: getCurrentUserId
    };
});